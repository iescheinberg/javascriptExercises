    ...
    validarArreglo(propiedad, valor) {
        ...
        for (const cadena of valor) {
            if (typeof cadena !== "string") {
                return console.error(`El valor ${cadena}, NO es un a cadena de texto`)
            }
        }
    ...
    validarIMBD(id) {
        if (this.validarCadena("IMBD id", id)) {
            if (!(/^([a-z]){2}([0-9]){7}$/.test(id))) {
                return console.error(`IMBD id ${id} no es valido, debe tener 9 caracteres, los 2 primeros letras minúsculas, los restantes 7 son numeros`)
            }
        }
    }
        ...
        if (this.validarArreglo("Géneros", generos)) {
            for (let genero of generos) {
                if (!Pelicula.listaGeneros.includes(genero)) {
                    console.error(`Gnénero(s) incorrectos "${generos.join(", ")}"`);
    ...
    validarCalificacion(calificacion) {
        if (this.validarNumero("Calificación", calificacion))
            return (calificacion < 0 || calificacion > 10)
                ? console.error('La calificación debe estar en un rango de 0 y 10')
